    ...
    public abstract class ExtenderControlBase : ExtenderControl, IControlResolver
    {
        ...
        internal IScriptService EnsureProfileScriptService(ScriptManager sm, bool checkIfNeeded)
        {
            ...
            if (sm != null)
            {
                ...
                bool needScriptService = false;

                if (checkIfNeeded)
                {
                    
                    if (ProfileBindings.Count > 0)
                    {
                        needScriptService = true;
                        break;
                    }                
                }
                else
                ...
                if (needScriptService)
                {

                    ProfileServiceManager psm = sm.ProfileService;

                    IScriptService profileService = null;


                    try
                    {
                        profileService = sm.GetScriptService(typeof(ProfileScriptService));
                    }
                    catch(KeyNotFoundException)
                    {
                        // Entry not already present; will add one below
                    }

                    if (profileService == null)
                    {
                        ProfileScriptService pss = new ProfileScriptService();
                        pss.AutoSave = true;
                        pss.ID = "profileScriptService";
                        Controls.Add(pss);
                        profileService = pss;
                    }
                }
