...
const _logger = debug.extend('JobPreview');
...
const JobPreview = (props) => {
    ...
    useEffect(() => {
        lookUpService.LookUp(['States', 'LocationTypes', 'JobTypes']).then(onSuccessLookup).catch(onErrorLookup);
    }, []);
    ...
    const onSuccessLookup = (response) => {
        ...
        setJobTypes(response.item.jobTypes);
        setLookUpDone(true);
    };
    ...
    const getJobType = () => {
        const jobType = jobTypes.filter((jobType) => jobType.id === Number(props.formData.jobTypeId));
        ...
        <React.Fragment>
            <Form onSubmit={handleSubmit} className="p-1">
                <Card className="p-4 mx-auto jobs-form-card-width">
                    <Card.Header>
                        <h2 className="text-center">Job Preview</h2>
                    </Card.Header>
                    <h4 className="text-left">Job Information:</h4>
                    <div className="form-group mt-2">
                        <div className="row p-2">
                            <div className="col-sm-6">
                                <p>
                                    <strong>Job Type: </strong>
                                    {lookUpDone && getJobType()}
                                </p>
                            </div>
                        </div>
                        <div className="row p-2">
