...
const SurveySearchList = () => {
    ...
    const onSearchBtnClick = (e) => {
        ...
        if (searchData.query === '') {
            surveysInstanceService
                .getAllInsightsPagination(tableState.pageIndex, tableState.pageSize)
                .then(onGetInsightsSuccess)
                .catch(onGetInsightsFail);
        } else {
            surveysInstanceService
                .searchInsights(tableState.pageIndex, tableState.pageSize, searchData.query)
                .then(onGetInsightsSuccess)
    ...
    const onResultCardClick = (id) => {
        navigate(`/admin/surveys/insights/${id}`);
    };
    ...
    const onPaginationClicked = (page) => {
        ...
        if (searchData.query === '') {
            surveysInstanceService
                .getAllInsightsPagination(pageIdx, tableState.pageSize)
                .then(onGetInsightsSuccess)
                .catch(onGetInsightsFail);
        } else {
            surveysInstanceService
                .searchInsights(pageIdx, tableState.pageSize, searchData.query)
                .then(onGetInsightsSuccess)
                .catch(onSearchInsightsFail);
        }
    };
                    ...
                    <Row>
                        ...
                        <Col md={5}>
                            <div>
                                ...
                                <p className="font-16 surveys-list-data">Questions: {insight.insightData?.length}</p>
                                <p className="font-16 surveys-list-data">Responses: {insight.instanceCount}</p>
                        ...
                        <Col md={1}>
                            <div className="float-end">
                                <img src={insight.logo} alt="" className="img-fluid" />
                            </div>
                        </Col>
                    </Row>
                </Card.Body>
                ...
                <Card.Body>
                    <Row>
                        <Col className="float-start">
                            <h3>Surveys</h3>
                            <p className="text-muted">Click on a card to view survey insights</p>
                        </Col>
                        <Col>
                            <SearchForm />
                        </Col>
